...
{
	struct Command *os_command;
	LIST_FOREACH(os_command, &foundCommands) {
	    LIST_REMOVE(&foundCommands, os_command);// to clear foundCommands list
	}
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++) {
	    if (strcmp(arguments[0], commands[i].name) == 0) {
	        ...
	        if (((commands[i].num_of_args == -1) && (number_of_arguments >= 1)) ||
	            ...
	            struct Command *addedelement1;
	            addedelement1 = &commands[i];
	            LIST_INSERT_HEAD(&foundCommands, addedelement1);
	            return CMD_INV_NUM_ARGS;
	        }
	    }
	}

	struct Command *matched_command_will_addtofoundlist;


	for (int i = 0; i < NUM_OF_COMMANDS; i++) {// loop on all commandes in fos
	    int numberoffoundercharsinorder=0;//counter on number of founder char
	    int indexincommand=0;
	    for (int j = 0; arguments[0][j] != '\0'; j++) { // loop on the command user enter
	    	for (int k =indexincommand;k<strlen(commands[i].name);k++)// loop on chars in the command
	    	{
	    		indexincommand++;
	    		if(commands[i].name[k]==arguments[0][j])
	    		{
	    			numberoffoundercharsinorder++;
	    			break;
	    		}
	    	}
	    }
	    if (numberoffoundercharsinorder==strlen(arguments[0])) {// if command match with the user command i will put in list
	    	matched_command_will_addtofoundlist = &commands[i];
	        LIST_INSERT_HEAD(&foundCommands,matched_command_will_addtofoundlist);
	    }
	}

	// If there are any matched commands, print them and return CMD_MATCHED
